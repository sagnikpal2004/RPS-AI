...
namespace Rock__paper_and_scissors___AI {
    /// <summary>
    /// Contains all the methods that runs the AI
    /// </summary>
    ...
    public sealed partial class MainPage : Page {
        ...
        /*//Inputs the probability counts from CSV
        public async System.Threading.Tasks.Task<int> InputFromCSV(string pattern, string pattern2 = "") {
            Windows.Storage.StorageFolder storageFolder = Windows.Storage.ApplicationData.Current.LocalFolder;
            Windows.Storage.StorageFile dataset = await storageFolder.GetFileAsync("dataset.csv");
            Debug.WriteLine("The program has read from the file !!!");

            string text = await Windows.Storage.FileIO.ReadTextAsync(dataset);

            int i = -1, j;
            while (i < text.Length - 1) {
                string line = "", input = "", input2 = "";
                for (i++; text[i] != '\n'; i++) line += text[i];
                for (j = 0; line[j] != ','; j++) input += line[j];
                for (j++; line[j] != ','; j++) input2 += line[j];
                ...
                if (input == pattern && input2 == pattern2) {
                    ...
                    for (j = pattern.Length * 2 + 2; j < line.Length; j++) countS += line[j];
                    return Int32.Parse(countS);
                }
            }
            return 0;
        }

        //Inputs the probability counts from CSV - OVERLOAD
        public async System.Threading.Tasks.Task<int> InputFromCSV(string pattern) {
            Windows.Storage.StorageFolder storageFolder = Windows.Storage.ApplicationData.Current.LocalFolder;
            Windows.Storage.StorageFile dataset = await storageFolder.CreateFileAsync("dataset.csv", CreationCollisionOption.OpenIfExists);
            ...
            while (i < text.Length - 1)
            {
                string line = "", input = "";
                for (i++; text[i] != '\n'; i++) line += text[i];
                for (int j = 0; line[j] != ','; j++) input += line[j];

                if (input == pattern)
                {
                    string countS = "";

                    for (int j = pattern.Length * 2 + 2; j < line.Length; j++) countS += line[j];
                    count += Int32.Parse(countS);
                }
            }
            return count;
